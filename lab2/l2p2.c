 ...
 * repeatedly. This version uses system calls such as open, read, close, etc. without
 * a buffer.
 * 09/14/2020 */
...
int main()
{
	...
	while(read(f, &c, 1) != 0)
	{
		...
		for(i = 0; i < names; i++)
		{
			...
			long name_beg = lseek(f, 0, SEEK_CUR);
			...
			for(j = 0;; j++)
			{
				...
				if(lseek(f, 0, SEEK_CUR) <= dot_mark)
				{
					prefix_name[j] = c;
